...
try {
  ...
  // Read workflow state if active feature exists
  const state = feature ? getWorkflowState(feature) : null;
  ...
  const contextString = [
    ...
    '2. Continue from your current gate — do NOT restart from Gate 1',
    '3. The hooks WILL block you if you try to skip gates',
    '</workflow-enforcement>'
  ].join('\n');
