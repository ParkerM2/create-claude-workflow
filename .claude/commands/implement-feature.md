...
If a design document or spec exists for this feature, read it too. Check for a plan generated by `/create-feature-plan`:
...
4. Suggest `/resume-feature` instead of starting fresh
...
3. The `current.md` and `history.md` files will be auto-generated from events

The `events.jsonl` file is your **crash-recovery artifact**. Events are appended automatically by hooks and via `/track` commands.
