 ...
 *   claudeDir: { exists: bool, commands: string[], agents: string[], prompts: string[], hooks: string[], settings: object|null },
...
function detectClaudeDir(projectRoot) {
  ...
  if (!fs.existsSync(claudeDir)) {
    return { exists: false, commands: [], agents: [], prompts: [], hooks: [], settings: null };
  }
  ...
  const hooks = listFilesByExt(path.join(claudeDir, 'hooks'), '.js');
  return { exists: true, commands, agents, prompts, hooks, settings };
}
