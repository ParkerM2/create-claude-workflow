...
2. Read `{{ARCHITECTURE_FILE}}` — system architecture (if it exists)

### Step 2: Scan for Active Features
1. Scan `{{PROGRESS_DIR}}/` for progress files
...
1. Check if `{{PROGRESS_DIR}}/ideas-backlog.md` exists
...
Create `{{PROGRESS_DIR}}/<feature-name>-progress.md` using the template from `.claude/prompts/implementing-features/PROGRESS-FILE-TEMPLATE.md`.
...
Create `{{PROGRESS_DIR}}/<feature-name>-design.md` with:
...
- Scan `{{PROGRESS_DIR}}/` for a feature with status `IN_PROGRESS`
...
Create `{{PROGRESS_DIR}}/<name>-design.md`:
...
If `{{PROGRESS_DIR}}/ideas-backlog.md` doesn't exist, create it using the template from `.claude/prompts/implementing-features/IDEAS-BACKLOG-TEMPLATE.md`.
...
### Step 3: Append Entry
Append to `{{PROGRESS_DIR}}/ideas-backlog.md`:
