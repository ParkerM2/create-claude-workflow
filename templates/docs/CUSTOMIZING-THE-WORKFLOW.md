...
Open `.claude/prompts/implementing-features/AGENT-SPAWN-TEMPLATES.md` and edit the Standard Coding Agent Spawn template.
...
- `.claude/agents/qa-reviewer.md` (Rules section, item 7)
- `.claude/prompts/implementing-features/README.md` (Section 7: QA Round Limits)
...
- `.claude/prompts/implementing-features/README.md` (Section 2)
- `.claude/agents/team-leader.md` (Branching Model section)
- `.claude/commands/implement-feature.md` (Phases 4, 6)
...
The `{{PROGRESS_DIR}}` variable (default: `docs/progress`) is used everywhere. To change it:

1. Run `npx create-claude-workflow init` and specify a different directory
2. Or manually update `{{PROGRESS_DIR}}` references in:
...
Edit `.claude/prompts/implementing-features/PROGRESS-FILE-TEMPLATE.md` to add sections:
...
In `.claude/prompts/implementing-features/README.md` (Section 8) and `team-leader.md`:
...
See [`QA-CHECKLIST-AUTO-FILL-RULES.md`](../prompts/implementing-features/QA-CHECKLIST-AUTO-FILL-RULES.md) for rules that map agent roles to QA checklist sections. This reduces boilerplate — the Team Leader only needs to add feature-specific checks.

---

## 6. Template Variable Reference

All template files use `{{VARIABLE}}` placeholders that are replaced during scaffolding.

| Variable | Default | Used In | Purpose |
|----------|---------|---------|---------|
| `{{PROJECT_NAME}}` | Directory name | Progress file | Project identifier |
| `{{PROJECT_RULES_FILE}}` | `CLAUDE.md` | All agents, commands | Path to project rules |
| `{{ARCHITECTURE_FILE}}` | `docs/ARCHITECTURE.md` | All agents, commands | Path to architecture doc |
| `{{PROGRESS_DIR}}` | `docs/progress` | Commands, playbook, agents | Progress file directory |
| `{{AGENT_ROLE}}` | Per agent | Agent definitions | Agent's role name |
| `{{AGENT_FILE_SCOPE}}` | Per agent | Agent definitions | Files the agent may modify |
| `{{AGENT_EXCLUDED_FILES}}` | Per agent | Agent definitions | Files the agent must not touch |

### Adding Custom Variables

1. Define the variable in your scaffolding config or replace manually after install
2. Use `{{YOUR_VARIABLE}}` syntax in any template file
3. Common additions:
   - `{{TEST_COMMAND}}` — `npm run test` or `pytest` or `go test ./...`
   - `{{LINT_COMMAND}}` — `npm run lint` or `ruff check` or `golangci-lint run`
   - `{{BUILD_COMMAND}}` — `npm run build` or `cargo build` or `go build`
   - `{{DEFAULT_BRANCH}}` — `main` or `master` or `develop`
...
**Python project**:
```
{{PROJECT_RULES_FILE}} = pyproject.toml  (or a CLAUDE.md that references it)
{{TEST_COMMAND}} = pytest
...
The workflow supports three modes that control how much ceremony is applied. See [`WORKFLOW-MODES.md`](../prompts/implementing-features/WORKFLOW-MODES.md) for full details.
...
Set the mode in `{{PROJECT_RULES_FILE}}`:
```markdown
## Workflow Mode
Mode: standard
```
