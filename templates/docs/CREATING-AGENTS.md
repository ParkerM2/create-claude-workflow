...
> Guide for creating new specialist agent definitions and maintaining existing ones. Every agent is a plain `.md` file in `.claude/agents/` — you own it, you can customize it.
...
4. `.claude/prompts/implementing-features/README.md` — Playbook
```
...
1. Save the file as `.claude/agents/<role-name>.md`
2. The agent is immediately available for spawning by the Team Leader
3. No other registration is needed — the Team Leader reads `.claude/agents/` to discover available agents
...
- New conventions added to `{{PROJECT_RULES_FILE}}`
